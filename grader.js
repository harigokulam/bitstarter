...
var checkHtmlFile = function(htmlfile, checksfile,url) {
	if ( url) {
		restler.get(url).on( 'complete' ,function(result){
			verifyFile(cheerio.load(result), checksfile);
		});
	}else {
    		$ = cheerioHtmlFile(htmlfile);
		verifyFile($,checksfile);
	}
};
var verifyFile = function(html, checksfile) {
...
if(require.main == module) {
    ...
    .option('-u, --url <url>', 'location url', clone(assertUrlExists))
